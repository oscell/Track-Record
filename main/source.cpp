...
class recognition : public movement
{
	...
	int vision()
	{
		...
		// Capturing the frames in grayscale to enable better face detection
		cv::Mat gray;
		...
		while (cap.read(gray))
		{
			...
			int x_frame_centr = (gray.cols*0.5); // Cap width div by 2
			int y_frame_centr = (gray.rows*0.5); // Cap height div by 
